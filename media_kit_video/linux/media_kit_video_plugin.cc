...
#define MEDIA_KIT_VIDEO_PLUGIN(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), media_kit_video_plugin_get_type(), MediaKitVideoPlugin))
...
static void media_kit_video_plugin_handle_method_call(MediaKitVideoPlugin* self, FlMethodCall* method_call) {
  ...
  if (g_strcmp0(method, "VideoOutputManager.Create") == 0) {
    ...
    gint64 handle_value = g_ascii_strtoll(fl_value_get_string(handle), NULL, 10);
    gint64 width_value = 0;
    gint64 height_value = 0;
    if (g_strcmp0(fl_value_get_string(width), "null") != 0 && g_strcmp0(fl_value_get_string(height), "null") != 0) {
    ...
    VideoOutputTextureUpdateCallbackData* data = g_new0(VideoOutputTextureUpdateCallbackData, 1);
    ...
    video_output_manager_create(
        ...
        [](gint64 id, gint64 width, gint64 height, gpointer context) {
          ...
          fl_method_channel_invoke_method(channel, "VideoOutput.Resize", result, NULL, NULL, NULL);
        },
        data);
...
static MediaKitVideoPlugin* media_kit_video_plugin_new(FlPluginRegistrar* registrar) {
  MediaKitVideoPlugin* self = MEDIA_KIT_VIDEO_PLUGIN(g_object_new(media_kit_video_plugin_get_type(), nullptr));
  g_autoptr(FlMethodCodec) codec = FL_METHOD_CODEC(fl_standard_method_codec_new());
  self->channel =
      fl_method_channel_new(fl_plugin_registrar_get_messenger(registrar), "com.alexmercerind/media_kit_video", codec);
  fl_method_channel_set_method_call_handler(self->channel, method_call_cb, self, g_object_unref);
  FlTextureRegistrar* texture_registrar = fl_plugin_registrar_get_texture_registrar(registrar);
  // Create new |VideoOutputManager| instance. Pass |texture_registrar| as reference.
  self->video_output_manager = video_output_manager_new(texture_registrar);
  return self;
}

void media_kit_video_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
